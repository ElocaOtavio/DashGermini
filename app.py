import streamlit as stimport pandas as pdimport requestsfrom io import BytesIOimport plotly.express as pximport plotly.graph_objects as goimport osfrom datetime import datetime
# --- Configuração da Página ---st.set_page_config(
    page_title="Dashboard de Indicadores",
    page_icon="📊",
    layout="wide")
# --- Funções de Carregamento e Tratamento de Dados ---@st.cache_data(ttl=3600) # Cache de 1 horadef carregar_dados_operacionais(url, headers):"""Carrega e trata os dados operacionais da Eloca."""try:
        resposta = requests.get(url, headers=headers)        resposta.raise_for_status()  # Lança um erro para códigos de status ruins (4xx ou 5xx)        arquivo = BytesIO(resposta.content)        df = pd.read_excel(arquivo)                # --- Limpeza e Conversão de Tipos ---# Converte colunas de data/horafor col in ['Data de Criação', 'Data da Primeira Resposta', 'Data da Resolução', 'Data do Primeiro Atendimento', 'Data do Segundo Atendimento']:
            if col in df.columns:
                df[col] = pd.to_datetime(df[col], errors='coerce')

        # Converte colunas de tempo para numérico (minutos)for col in df.columns:
        ...
        return pd.DataFrame()
@st.cache_data(ttl=3600) # Cache de 1 horadef carregar_dados_csat(url, headers):"""Carrega, trata e aplica a regra de desduplicação nos dados de CSAT."""try:
        resposta = requests.get(url, headers=headers)        resposta.raise_for_status()        arquivo = BytesIO(resposta.content)        df = pd.read_excel(arquivo)
        # Renomear coluna de avaliação para facilitar o acesso        coluna_avaliacao = next((col for col in df.columns if 'Como você avalia a qualidade' in col), None)
        if not coluna_avaliacao:
            ...
            return df
                ...
                # --- Lógica de Desduplicação do CSAT ---        df['Avaliacao_Qualidade'] = df['Avaliacao_Qualidade'].astype(str)
                # 1. Ordenar para priorizar as melhores avaliações        df['prioridade_avaliacao'] = df['Avaliacao_Qualidade'].apply(
            lambda x: 1 if x.startswith('Ótimo') else (2 if x.startswith('Bom') else 3)
                ...
                # 2. Manter apenas a primeira ocorrência após a ordenação        df_final = df_sorted.drop_duplicates(subset='Código do Chamado', keep='first')
        ...
        return pd.DataFrame()
# --- Carregamento dos Dados Usando Secrets ---# Use st.secrets para um ambiente de produção no Streamlit Cloud# Para teste local, pode usar os valores diretamente ou um arquivo .envURL_OPERACIONAL = st.secrets.get("ELOCA_URL", os.getenv("ELOCA_URL"))
...
# --- Barra Lateral de Filtros ---st.sidebar.header("Filtros Globais")
# Filtro de Períodoif not df_operacional.empty and 'Data de Criação' in df_operacional.columns:
    data_min = df_operacional['Data de Criação'].min().date()
    data_max = df_operacional['Data de Criação'].max().date()
        data_selecionada = st.sidebar.date_input(        "Selecione o Período",
        value=(data_min, data_max),        min_value=data_min,        max_value=data_max,    )    if len(data_selecionada) == 2:
        start_date, end_date = pd.to_datetime(data_selecionada[0]), pd.to_datetime(data_selecionada[1])
        df_operacional = df_operacional[df_operacional['Data de Criação'].between(start_date, end_date)]else:
    st.sidebar.warning("Dados operacionais ou coluna 'Data de Criação' não disponíveis para filtro.")
# Filtro de Analistaif not df_operacional.empty and 'Analista da Resolução' in df_operacional.columns:
    lista_analistas = sorted(df_operacional['Analista da Resolução'].dropna().unique())
    analista_selecionado = st.sidebar.multiselect(        "Selecione o(s) Analista(s)",
        options=lista_analistas,        default=lista_analistas    )    df_operacional = df_operacional[df_operacional['Analista da Resolução'].isin(analista_selecionado)]else:
    st.sidebar.warning("Coluna 'Analista da Resolução' não disponível para filtro.")

# --- Navegação Principal ---st.sidebar.title("Navegação")
paginas = [
...
# --- Conteúdo das Páginas ---if pagina_selecionada == "Resultados Área 1 (TMA, TME, TMR)":
    st.title("📈 Resultados Área 1: Tempo Médio de Atendimento, Espera e Resolução")
        if not df_operacional.empty:
        # Cálculos dos KPIs        tme = df_operacional['Tempo Útil até o primeiro atendimento'].mean()
        ...
        st.subheader("Evolução Diária dos Tempos Médios")
            ...
            'Tempo Útil da Resolução': 'mean'        }).reset_index()
        fig = go.Figure()        fig.add_trace(go.Bar(x=df_diario['Data de Criação'], y=df_diario['Tempo Útil até o primeiro atendimento'], name='TME'))
        ...
        fig.update_layout(            title="TME, TMA e TMR por Dia",
            xaxis_title="Data",
            yaxis_title="Tempo (minutos)",
            yaxis2=dict(title="TMR (minutos)", overlaying='y', side='right'),
            legend_title="Métricas",
            barmode='group'        )        st.plotly_chart(fig, use_container_width=True)
    else:
    ...
    st.title("😊 Resultados Área 2: Satisfação do Cliente (CSAT)")
        ...
        # Merge para obter o analista        df_csat_merged = pd.merge(df_csat, df_operacional[['Código', 'Analista da Resolução']], left_on='Código do Chamado', right_on='Código', how='left')

        # Filtra CSAT com base nos analistas selecionados no filtro global        df_csat_merged = df_csat_merged[df_csat_merged['Analista da Resolução'].isin(analista_selecionado)]
                total_respostas = len(df_csat_merged)
                # Extrai a nota da avaliação (assumindo que a nota é o primeiro caractere)        df_csat_merged['Nota'] = pd.to_numeric(df_csat_merged['Avaliacao_Qualidade'].str[0], errors='coerce')
                media_notas = df_csat_merged['Nota'].mean()
                # Considera "satisfeitos" quem deu nota 4 ou 5        satisfeitos = df_csat_merged[df_csat_merged['Nota'].isin([4, 5])].shape[0]
    ...
    st.title("📅 Gráfico Individual 1: Desempenho Diário por Analista")
        ...
        ).reset_index()                fig = px.scatter(df_agrupado, x='Data de Criação', y='Chamados_Resolvidos', size='TMA', color='Analista da Resolução',
    ...
    # --- Modelo de Metas ---# Em um cenário real, isso viria de um arquivo ou banco de dados
    metas = {
        'TMA': 30, # minutos'TME': 15, # minutos'CSAT': 4.5 # nota média
    }
    ...
    # Função para formatar visualmentedef formatar_meta(df):def colorir_booleano(val):
            color = 'lightgreen' if val else 'lightcoral'return f'background-color: {color}'return df.style.applymap(colorir_booleano, subset=['Atingiu_TMA', 'Atingiu_TME', 'Atingiu_CSAT']) \
                       .format({
                           'TMA_Realizado': '{:.2f}',
                           'TME_Realizado': '{:.2f}',
                           'CSAT_Realizado': '{:.2f}'
